...
3. During work, save only stable decisions, constraints, verified evidence, and expensive-to-rediscover failures. Use stable topic keys so newer facts supersede older ones.
4. Before context becomes crowded or when a task reaches a meaningful boundary, call `memory_checkpoint`. Write terse model-readable facts rather than conversational prose.
5. Before final delivery, checkpoint unresolved next steps if continuation is likely. Do not claim a checkpoint was saved unless the tool succeeds.
